...
export default function AuthModal({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) {
  ...
  useEffect(() => {
    if (status === 'authenticated' && isOpen) {
      ...
      // Rediriger vers l'URL actuelle ou /profile par défaut
      router.push(pathname || '/profile')
    }
  }, [status, isOpen, onClose, router, pathname])
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result?.ok) {
          ...
          // Utiliser returnTo, puis l'URL actuelle, puis /profile par défaut
          router.push(returnTo || pathname || '/profile')
        } else {
            ...
            <>
              ...
              <div className={styles.socialButtons}>
                <button
                  ...
                  onClick={() => signIn('discord', { callbackUrl: pathname || '/profile' })}
                >
                ...
                <button
                  ...
                  onClick={() => signIn('google', { callbackUrl: pathname || '/profile' })}
                >
                  <svg className={styles.socialIcon} viewBox="0 0 24 24" fill="currentColor">
