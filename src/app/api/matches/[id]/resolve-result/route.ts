...
// POST: Permettre à l'admin de trancher un résultat de match
export async function POST(
  ...
  try {
    ...
    const { winnerTeamId } = await request.json()

    if (!winnerTeamId) {
      return NextResponse.json({ message: 'winnerTeamId requis' }, { status: 400 })
    }

    // Vérifier que l'utilisateur est l'admin du tournoi
    const match = await prisma.match.findUnique({
      where: { id: matchId },
      include: {
        tournament: {
    ...
    if (match.tournament.organizer.id !== userId) {
      return NextResponse.json({ message: 'Seul l\'admin du tournoi peut trancher' }, { status: 403 })
    }

    // Vérifier que le winnerTeamId correspond à teamA ou teamB
    if (winnerTeamId !== match.teamAId && winnerTeamId !== match.teamBId) {
      return NextResponse.json({ message: 'winnerTeamId doit correspondre à teamA ou teamB' }, { status: 400 })
    }
    ...
    console.error('Resolve match result error:', error)
